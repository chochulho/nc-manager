    ...
    <div>
      <div className="page-header">
        <div className="flex items-center gap-3">
          ...
          <h1 className="page-title">공급자 관리</h1>
        </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-5 space-y-4">
          <h2 className="section-title">{editing ? "공급자 수정" : "신규 공급자"}</h2>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label>이름 *</Label>
              <Input className="mt-1" value={form.name} onChange={(e) => setForm(p => ({ ...p, name: e.target.value }))} placeholder="(주)예시부품" />
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-muted-foreground">등록된 공급자가 없습니다.</div>
        ) : (
