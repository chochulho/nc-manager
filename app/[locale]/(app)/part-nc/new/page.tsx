...
import { NewPartNCForm } from "./new-part-nc-form";

export default async function NewPartNCPage() {
  const session = await auth();
  if (!session?.user?.organizationId) redirect("/dashboard");

  const orgId = session.user.organizationId;

  const [sites, processes, parts, suppliers, categoriesL2, defaultSiteId] = await Promise.all([
  ...
  return <NewPartNCForm sites={sites} processes={processes} parts={parts} suppliers={suppliers} categoriesL2={categoriesL2} defaultSiteId={defaultSiteId} />;
}
