...
export default async function DashboardPage({
  ...
  const [session, t, tNc, tPnc, tCc, tc, tPeriod] = await Promise.all([
    auth(),
  ...
  const [ncItems, pncItems, ccItems, capaItems] = await Promise.all([
    ncConditions
      ? db
    ...
    pncConditions
      ? db
          .select({ severity: partNCs.severity, status: partNCs.status })
          .from(partNCs)
          .where(and(...pncConditions))
      : Promise.resolve([]),
    ccConditions
      ? db
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <table className="w-full text-sm mb-4">
              ...
              <tbody>
                {severityKeys.map((s) => {
                  const cnt = pncBySeverity[s] ?? 0;
                  const pct = pncTotal > 0 ? Math.round((cnt / pncTotal) * 100) : 0;
                  return (
                    <tr key={s} className="border-b last:border-0">
                      <td className="py-1.5 text-xs">{tPnc(`severities.${s}`)}</td>
                      <td className="py-1.5 text-right font-semibold">{cnt}</td>
                      <td className="py-1.5 text-right text-muted-foreground text-xs">{pct}%</td>
                    </tr>
                  );
                ...
                <tr className="font-semibold">
                  <td className="py-1.5 text-xs">{tc("total")}</td>
                  <td className="py-1.5 text-right">{pncTotal}</td>
                  <td className="py-1.5 text-right text-muted-foreground text-xs">100%</td>
                </tr>
              </tbody>
            </table>
