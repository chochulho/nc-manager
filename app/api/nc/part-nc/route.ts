...
import { partNCs, ncSequences } from "@/lib/db/schema";
import { eq, and } from "drizzle-orm";
import { sql } from "drizzle-orm";
import { buildSiteFilter, getSelectedSiteId } from "@/lib/site-filter";

async function nextPncNumber(orgId: string): Promise<string> {
  const year = new Date().getFullYear();
  const result = await db
    .insert(ncSequences)
    .values({ orgId, entityType: "part_nc", year, lastSeq: 1 })
    .onConflictDoUpdate({
  ...
  return `PNC-${year}-${String(seq).padStart(4, "0")}`;
}
...
export async function GET(req: NextRequest) {
  ...
  const items = await db
    .select()
    .from(partNCs)
    .where(and(eq(partNCs.orgId, session.user.organizationId), siteFilter))
...
export async function POST(req: NextRequest) {
  ...
  const pncNumber = await nextPncNumber(session.user.organizationId);
