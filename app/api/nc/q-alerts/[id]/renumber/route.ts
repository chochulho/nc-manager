...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [existing] = await db
    .select({ id: qAlerts.id, alertNumber: qAlerts.alertNumber })
    .from(qAlerts)
    .where(and(eq(qAlerts.id, id), eq(qAlerts.orgId, session.user.organizationId)));
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const result = await renumberEntity("q_alert", id, session.user.organizationId, year, seq);
  if (!result.ok) return NextResponse.json({ error: result.error }, { status: 409 });

  await logAdminAction(session.user.organizationId, "q_alert", id, session.user.id, "renumbered", {
    oldNumber: existing.alertNumber,
